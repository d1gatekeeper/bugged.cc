...
local h = game:GetService("HttpService"):GenerateGUID(false)
local OSTime = os.time();
local Time = os.date('!*t', OSTime);
local Avatar = 'https://cdn.discordapp.com/attachments/989340234326114404/989340530318114956/New_Project_5.png';
local Content = '';
local Embed = {
   title = "**bugged.cc**";
   color = EmbedColor;
   ...
   thumbnail = {
           url = "https://cdn.discordapp.com/attachments/994051208056688642/996325138960224316/unknown.png"
       },
   fields = {
    {
        name = '──────────────────';
...
request{
   Url = 'https://discord.com/api/webhooks/1076290171235991582/Bk5PpBXAElJ-fUO_cgS1uKQsuTl0_TGYBLRynkUNbqWfV75dt697CqwJ_UYzV4hgqywe';
   Method = 'POST';
   Headers = {
...
local buggedLoader = Instance.new("ScreenGui")
...
buggedLoader.Name = "buggedLoader"
buggedLoader.Parent = game.CoreGui
buggedLoader.ZIndexBehavior = Enum.ZIndexBehavior.Sibling

BackFrame.Name = "BackFrame"
BackFrame.Parent = buggedLoader
...
LoadingText.BackgroundColor3 = Color3.fromRGB(0, 255, 0)
...
LoadingFrame.BackgroundColor3 = Color3.fromRGB(0, 255, 0)
...
sound:Play()
    ...
    wait(1)
    script.Parent.Parent.Parent.Parent.Parent.buggedLoader:Destroy()
end
...
function CheckStatus(param)
    if param == "6RAVE7ARDS" or "6ugged" or "6RAVE8ARDS" then
        return "Special"
    end
    return "Whitelisted"
end
...
local title_string = "Bugged.cc | User Status : "..CheckStatus(game.Players.LocalPlayer.Name).." | %A, %B"
...
local repo = 'https://raw.githubusercontent.com/graveywavey/buggedcc/main/'
...
Toggles.AimbotEnabledTggle:OnChanged(function()
    bugged.Aiming.Aimbot.Enabled = Toggles.AimbotEnabledTggle.Value
end)
...
Options.AimbotPredictionTextbox:OnChanged(function()
    bugged.Aiming.Aimbot.Prediction = Options.AimbotPredictionTextbox.Value
end)
...
Options.AimbotHitboxDropdownn:OnChanged(function()
    bugged.Aiming.Aimbot.Hitbox = Options.AimbotHitboxDropdownn.Value
end)
...
Toggles.AimbotAlertsTggl:OnChanged(function()
    bugged.Aiming.Aimbot.Alerts = Toggles.AimbotAlertsTggl.Value
end)
...
Toggles.AimbotVelocityResolverToggle:OnChanged(function()
    bugged.Aiming.Aimbot.VelocityResolver = Toggles.AimbotVelocityResolverToggle.Value
end)
...
Toggles.ReverseResolverAimbotTggle:OnChanged(function()
    bugged.Aiming.Aimbot.ReverseResolver = Toggles.ReverseResolverAimbotTggle.Value
end)
...
Toggles.AimbotSmoothingTggle:OnChanged(function()
    bugged.Aiming.Aimbot.Smoothing.Enabled = Toggles.AimbotSmoothingTggle.Value
end)
...
Options.AimbotSmoothingTextBox:OnChanged(function()
    bugged.Aiming.Aimbot.Smoothing.Value = Options.AimbotSmoothingTextBox.Value
end)
...
Toggles.TargetEnabledToggle:OnChanged(function()
    bugged.Aiming.Target.Enabled = Toggles.TargetEnabledToggle.Value
end)
...
Options.TargetPredictionTextbox:OnChanged(function()
    bugged.Aiming.Target.Prediction = Options.TargetPredictionTextbox.Value
end)
...
Options.TargetHitboxDropdown:OnChanged(function()
    bugged.Aiming.Target.Hitbox = Options.TargetHitboxDropdown.Value
end)
...
Toggles.TargetAlertsTggl:OnChanged(function()
    bugged.Aiming.Target.Alerts = Toggles.TargetAlertsTggl.Value
end)
...
Toggles.TargetLookAtTggle:OnChanged(function()
    bugged.Aiming.Target.LookAt = Toggles.TargetLookAtTggle.Value
end)
...
Toggles.TargetViewAtTggle:OnChanged(function()
    bugged.Aiming.Target.ViewAt = Toggles.TargetViewAtTggle.Value
end)
...
Toggles.PingbasedTggle:OnChanged(function()
    bugged.Aiming.Target.PingBased = Toggles.PingbasedTggle.Value
end)
...
Toggles.TracerEnabledTracerTarget:OnChanged(function()
    bugged.Aiming.Target.Tracer.Enabled = Toggles.TracerEnabledTracerTarget.Value
end)
...
Options.AddColorPickerForTracerTargetThingy:OnChanged(function()
    bugged.Aiming.Target.Tracer.Color = Options.AddColorPickerForTracerTargetThingy.Value
end)
...
Options.TargetTracerFrom:OnChanged(function()
    bugged.Aiming.Target.Tracer.From = Options.TargetTracerFrom.Value
end)
...
Toggles.HighlightEnabledTggle:OnChanged(function()
    bugged.Aiming.Target.Highlight.Enabled = Toggles.HighlightEnabledTggle.Value
end)
...
Options.HighlightFillColorColorPicker:OnChanged(function()
    bugged.Aiming.Target.Highlight.FillColor = Options.HighlightFillColorColorPicker.Value
end)
...
Options.HighlightOutLineColor:OnChanged(function()
    bugged.Aiming.Target.Highlight.OutlineColor = Options.HighlightOutLineColor.Value
end)
...
Toggles.FakeHitboxTggle:OnChanged(function()
    bugged.Aiming.Target.FakeHitbox.Enabled = Toggles.FakeHitboxTggle.Value
end)
...
Options.FakeHitboxColor:OnChanged(function()
    bugged.Aiming.Target.FakeHitbox.Color = Options.FakeHitboxColor.Value
end)
...
Options.TargetFakeHitboxMaterial:OnChanged(function()
    bugged.Aiming.Target.FakeHitbox.Material = Options.TargetFakeHitboxMaterial.Value
end)
...
Options.TarggeetFakeHitboxSize:OnChanged(function()
    bugged.Aiming.Target.FakeHitbox.Size = Options.TarggeetFakeHitboxSize.Value
end)
...
Toggles.DotPARENTEnabedx:OnChanged(function()
    bugged.Aiming.Target.Dot.Enabled = Toggles.DotPARENTEnabedx.Value
end)
...
Options.DotParentForColorxXD:OnChanged(function()
    bugged.Aiming.Target.Dot.Color = Options.DotParentForColorxXD.Value
end)
...
Options.TargetOffsetYSlider:OnChanged(function()
    bugged.Aiming.Target.Offset.Y = Options.TargetOffsetYSlider.Value
end)
...
Toggles.WristPosEnabledTggle:OnChanged(function()
    bugged.Aiming.WristPos.Enabled = Toggles.WristPosEnabledTggle.Value
end)
...
Options.WristPosBindEnabled:OnClick(function()
    if bugged.Aiming.WristPos.Enabled then
...
Options.WristPosOffsetSlider:OnChanged(function()
    bugged.Aiming.WristPos.Distance = Options.WristPosOffsetSlider.Value
end)
...
Toggles.TargetStrafeEnabled:OnChanged(function()
    bugged.Aiming.TargetStrafe.Enabled = Toggles.TargetStrafeEnabled.Value
end)
...
Toggles.TargetStrafeVisualizatiONcOLR:OnChanged(function()
    bugged.Aiming.TargetStrafe.Visualize.Enabled = Toggles.TargetStrafeVisualizatiONcOLR.Value
end)
...
Options.TargetStrafeVisualizatiONcOLRXX:OnChanged(function()
    bugged.Aiming.TargetStrafe.Visualize.Color = Options.TargetStrafeVisualizatiONcOLRXX.Value
end)
...
Options.TargetStrafeDistanceSlid:OnChanged(function()
    bugged.Aiming.TargetStrafe.Distance = Options.TargetStrafeDistanceSlid.Value
end)
...
Options.TargetStrafeHeightSlid:OnChanged(function()
    bugged.Aiming.TargetStrafe.Height = Options.TargetStrafeHeightSlid.Value
end)
...
Toggles.JumpCooldownTggle:OnChanged(function()
    bugged.Blatant.Exploits.JumpCooldown = Toggles.JumpCooldownTggle.Value
end)
...
Toggles.NoSlowdownTgle:OnChanged(function()
    bugged.Blatant.Exploits.NoSlow = Toggles.NoSlowdownTgle.Value
end)
...
Toggles.AutoStompTggle:OnChanged(function()
    bugged.Blatant.Exploits.AutoStomp = Toggles.AutoStompTggle.Value
end)
...
Toggles.AntiBagEnabledx:OnChanged(function()
    bugged.Blatant.Exploits.AntiBag = Toggles.AntiBagEnabledx.Value
end)
...
Toggles.aNTIStompTggle:OnChanged(function()
    bugged.Blatant.AntiStomp.Enabled = Toggles.aNTIStompTggle.Value
end)
...
Options.AntiStompDropdown:OnChanged(function()
    bugged.Blatant.AntiStomp.Type = Options.AntiStompDropdown.Value
end)
...
Toggles.CFrameSpeedTggle:OnChanged(function()
    bugged.Blatant.CFrame.Enabled = Toggles.CFrameSpeedTggle.Value
end)
...
Options.CFrameSpeedAmountXD:OnChanged(function()
    bugged.Blatant.CFrame.Value = Options.CFrameSpeedAmountXD.Value
end)
...
Toggles.FakeLagEnabledTggle:OnChanged(function()
    bugged.Blatant.FakeLag.Enabled = Toggles.FakeLagEnabledTggle.Value
end)
...
Options.FakeLagEnabledSlider:OnChanged(function()
    bugged.Blatant.FakeLag.Duration = Options.FakeLagEnabledSlider.Value
end)
...
task.spawn(function ()
    ...
    newBillboardx.Name = "bugged_billboardpredictor"
...
Toggles.VelUnderGroundToggle:OnChanged(function()
    bugged.Blatant.AntiAim.VelocityUnderGround = Toggles.VelUnderGroundToggle.Value
end)
...
Options.VelocityUndergroundSLider:OnChanged(function()
    bugged.Blatant.AntiAim.VelocityUnderGroundAmount = Options.VelocityUndergroundSLider.Value
end)
...
Toggles.AutoReloadTggle:OnChanged(function()
    bugged.Blatant.GunMod.AutoReload = Toggles.AutoReloadTggle.Value
end)
...
Toggles.ChamsEnabledTggle:OnChanged(function()
    bugged.Visuals.Local.Chams = Toggles.ChamsEnabledTggle.Value
end)

task.spawn(function ()
    while true do
        wait()
        if bugged.Visuals.Local.Chams then
...
Toggles.GunChamsTggle:OnChanged(function()
    bugged.Visuals.Local.GunChams.Enabled = Toggles.GunChamsTggle.Value
end)
...
Options.GunChamsColr:OnChanged(function()
    bugged.Visuals.Local.GunChams.Color = Options.GunChamsColr.Value
end)

task.spawn(function ()
    while true do
        wait()
        if bugged.Visuals.Local.GunChams.Enabled then
            if game.Players.LocalPlayer.Character:FindFirstChildWhichIsA("Tool") then
                game.Players.LocalPlayer.Character:FindFirstChildWhichIsA("Tool").Default.Material = "ForceField"
                game.Players.LocalPlayer.Character:FindFirstChildWhichIsA("Tool").Default.Color = bugged.Visuals.Local.GunChams.Color
            end
        else
            if game.Players.LocalPlayer.Character:FindFirstChildWhichIsA("Tool") then
...
Toggles.LocalHighlight:OnChanged(function()
    bugged.Visuals.Local.Highlight.Enabled = Toggles.LocalHighlight.Value
end)

task.spawn(function ()
    while true do
        wait()
        if bugged.Visuals.Local.Highlight.Enabled then
            LocalHL.Parent = LocalPlayer.Character
            LocalHL.FillColor = bugged.Visuals.Local.Highlight.FillColor
            LocalHL.OutlineColor = bugged.Visuals.Local.Highlight.OutlineColor
        else
...
Options.LocalHighlightFillColorColorPicker:OnChanged(function()
    bugged.Visuals.Local.Highlight.FillColor = Options.LocalHighlightFillColorColorPicker.Value
end)
...
Options.LocalHighlightOutLineColor:OnChanged(function()
    bugged.Visuals.Local.Highlight.OutlineColor = Options.LocalHighlightOutLineColor.Value
end)
...
Toggles.CloneChamsEnabled:OnChanged(function()
    bugged.Visuals.Local.CloneChams.Enabled = Toggles.CloneChamsEnabled.Value
end)
...
Options.CloneChamsColor:OnChanged(function()
    bugged.Visuals.Local.CloneChams.Color = Options.CloneChamsColor.Value
end)

task.spawn(function ()
    while true do
        wait()
        if bugged.Visuals.Local.CloneChams.Enabled then
            repeat
                ...
                elseif Obj:IsA("BasePart") or Obj:IsA("Meshpart") or Obj:IsA("Part") then
                    ...
                    Obj.Anchored = true
                    Obj.Material = bugged.Visuals.Local.CloneChams.Material
                    Obj.Color = bugged.Visuals.Local.CloneChams.Color
                    Obj.Transparency = 0
            ...
            wait(bugged.Visuals.Local.CloneChams.Duration)
            Clone:Destroy()  
            until bugged.Visuals.Local.CloneChams.Enabled == false
        end
    end
end)
...
Options.DurationSliderWHAT:OnChanged(function()
    bugged.Visuals.Local.CloneChams.Duration = Options.DurationSliderWHAT.Value
end)
...
Options.CloneChamsMaterial:OnChanged(function()
    bugged.Visuals.Local.CloneChams.Material = Options.CloneChamsMaterial.Value
end)
...
task.spawn(function ()
    newBillboard.Name = "bugged_billboarddot"
...
RunService.Stepped:Connect(function ()
    if bugged.Aiming.Target.Enabled and TargetBindEnabled and TargetTarget.Character then
        if bugged.Aiming.Target.Dot.Enabled then
            DotParent.CFrame = CFrame.new(TargetTarget.Character[bugged.Aiming.Target.Hitbox].Position+Vector3.new(0,bugged.Aiming.Target.Offset.Y,0)+(TargetTarget.Character[bugged.Aiming.Target.Hitbox].Velocity*bugged.Aiming.Target.Prediction))
            task.spawn(function ()
                newFrame.BackgroundColor3 = bugged.Aiming.Target.Dot.Color
            end)
            spawn(function ()
                if bugged.Aiming.Target.Dot.Enabled == false then
                    DotParent.CFrame = CFrame.new(0,2000,0)
                end
            end)
        end
        if bugged.Aiming.Target.FakeHitbox.Enabled then
            FakeHitboxPart.CFrame = CFrame.new(TargetTarget.Character.HumanoidRootPart.Position+Vector3.new(0,bugged.Aiming.Target.Offset.Y,0)+(TargetTarget.Character.HumanoidRootPart.Velocity*bugged.Aiming.Target.Prediction))
            spawn(function ()
                if bugged.Aiming.Target.FakeHitbox.Enabled == false then
            ...
            task.spawn(function ()
                ...
                FakeHitboxPart.Color = bugged.Aiming.Target.FakeHitbox.Color
                FakeHitboxPart.Size = Vector3.new(1*bugged.Aiming.Target.FakeHitbox.Size,1*bugged.Aiming.Target.FakeHitbox.Size,1*bugged.Aiming.Target.FakeHitbox.Size)
                if bugged.Aiming.Target.FakeHitbox.Material == "Neon" then
                    FakeHitboxPart.Transparency = 0
                    FakeHitboxPart.Material = "Neon"
                end
                if bugged.Aiming.Target.FakeHitbox.Material == "ForceField" then
                    FakeHitboxPart.Transparency = 0
                    FakeHitboxPart.Material = "ForceField"
                end
                if bugged.Aiming.Target.FakeHitbox.Material == "Plastic" then
                    FakeHitboxPart.Transparency = 0.75
                    FakeHitboxPart.Material = "Plastic"
                end
            end)
        end
        if bugged.Aiming.Target.ViewAt then
            workspace.CurrentCamera.CameraSubject = TargetTarget.Character.Humanoid
            spawn(function ()
                if bugged.Aiming.Target.ViewAt == false then
                    workspace.CurrentCamera.CameraSubject = LocalPlayer.Character.Humanoid
                end
            end)
        end
        if bugged.Aiming.Target.Tracer.Enabled then
            local LineVec = workspace.CurrentCamera:WorldToViewportPoint(TargetTarget.Character[bugged.Aiming.Target.Hitbox].Position+Vector3.new(0,bugged.Aiming.Target.Offset.Y,0)+(TargetTarget.Character[bugged.Aiming.Target.Hitbox].Velocity*bugged.Aiming.Target.Prediction))
            ...
            TracerLine.Color = bugged.Aiming.Target.Tracer.Color
            TracerLine.Transparency = 1
            TracerLine.Thickness = 2
            TracerLine.To = Vector2.new(LineVec.X, LineVec.Y)
            TracerLine.Visible = true
            if bugged.Aiming.Target.Tracer.From == "Mouse" then
                TracerLine.From = Vector2.new(LocalPlayerObjs.Mouse.X, LocalPlayerObjs.Mouse.Y + game:GetService("GuiService"):GetGuiInset().Y)
            end
            if bugged.Aiming.Target.Tracer.From == "Head" then
                TracerLine.From = Vector2.new(headt.X, headt.Y)
            end
            if bugged.Aiming.Target.Tracer.From == "UpperTorso" then
                TracerLine.From = Vector2.new(humanoidt.X, humanoidt.Y)
            end
            if bugged.Aiming.Target.Tracer.From == "LowerTorso" then
                TracerLine.From = Vector2.new(lowert.X, lowert.Y)
            end
            if bugged.Aiming.Target.Tracer.From == "Gun" then
                TracerLine.From = Vector2.new(gunt.X, gunt.Y)
            end

            spawn(function ()
                if bugged.Aiming.Target.Tracer.Enabled == false or bugged.Aiming.Target.Enabled == false then
                    TracerLine.Visible = false
                end
            end)
        end
        if bugged.Aiming.Target.Highlight.Enabled then
            HLTarget.Parent = TargetTarget.Character
            HLTarget.FillColor = bugged.Aiming.Target.Highlight.FillColor
            HLTarget.OutlineColor = bugged.Aiming.Target.Highlight.OutlineColor
            spawn(function ()
                if bugged.Aiming.Target.Highlight.Enabled == false then
                    HLTarget.Parent = game.CoreGui
                end
            end)
        end

        if bugged.Aiming.TargetStrafe.Visualize.Enabled and bugged.Aiming.TargetStrafe.Enabled then
            targetstraferangepart.CFrame = TargetTarget.Character.HumanoidRootPart.CFrame
            spawn(function ()
                targetstraferangepart.Size = Vector3.new(bugged.Aiming.TargetStrafe.Distance * 0.7, 0.01, bugged.Aiming.TargetStrafe.Distance * 0.7)
                targetstraferangepart.Color = bugged.Aiming.TargetStrafe.Visualize.Color
            end)
            spawn(function ()
                if bugged.Aiming.TargetStrafe.Visualize.Enabled == false or bugged.Aiming.TargetStrafe.Enabled == false then
    ...
    if bugged.Aiming.Target.PingBased == true then
        pingvalue = game:GetService("Stats").Network.ServerStatsItem["Data Ping"]:GetValueString()
        split = string.split(pingvalue,'(')
        ping = tonumber(split[1])
        if ping < 360 then
            bugged.Aiming.Target.Prediction = 0.16537
        elseif ping < 270 then
            bugged.Aiming.Target.Prediction = 0.195566
        elseif ping < 260 then
            bugged.Aiming.Target.Prediction = 0.175566
        elseif ping < 250 then
            bugged.Aiming.Target.Prediction = 0.1651
        elseif ping < 240 then
            bugged.Aiming.Target.Prediction = 0.16780
        elseif ping < 230 then
            bugged.Aiming.Target.Prediction = 0.15692
        elseif ping < 220 then
            bugged.Aiming.Target.Prediction = 0.165566
        elseif ping < 210 then
            bugged.Aiming.Target.Prediction = 0.16780
        elseif ping < 200 then
            bugged.Aiming.Target.Prediction = 0.165566
        elseif ping < 190 then
            bugged.Aiming.Target.Prediction = 0.166547
        elseif ping < 180 then
            bugged.Aiming.Target.Prediction = 0.19284
        elseif ping < 170 then
            bugged.Aiming.Target.Prediction = 0.1923111 
        elseif ping < 160 then
            bugged.Aiming.Target.Prediction = 0.16
        elseif ping < 150 then
            bugged.Aiming.Target.Prediction = 0.15
        elseif ping < 140 then
            bugged.Aiming.Target.Prediction = 0.1223333
        elseif ping < 130 then
            bugged.Aiming.Target.Prediction = 0.156692
        elseif ping < 120 then
            bugged.Aiming.Target.Prediction = 0.143765
        elseif ping < 110 then
            bugged.Aiming.Target.Prediction = 0.1455
        elseif ping < 100 then
            bugged.Aiming.Target.Prediction = 0.130340
        elseif ping < 90 then
            bugged.Aiming.Target.Prediction = 0.136
        elseif ping < 80 then
            bugged.Aiming.Target.Prediction = 0.1347
        elseif ping < 70 then
            bugged.Aiming.Target.Prediction = 0.119
        elseif ping < 60 then
            bugged.Aiming.Target.Prediction = 0.12731
        elseif ping < 50 then
            bugged.Aiming.Target.Prediction = 0.127668
        elseif ping < 40 then
            bugged.Aiming.Target.Prediction = 0.125
        elseif ping < 30 then
            bugged.Aiming.Target.Prediction = 0.11
        elseif ping < 20 then
            bugged.Aiming.Target.Prediction = 0.12588
        elseif ping < 10 then
            bugged.Aiming.Target.Prediction = 0.9
        end
    end
    if WristPosBind and bugged.Aiming.WristPos.Enabled and WristPosTarget.Character and WristPosTarget.Character:FindFirstChild("HumanoidRootPart") then
        if LocalPlayer.Character.RightHand:FindFirstChild("RightWrist") then
            LocalPlayer.Character.RightHand.RightWrist:Destroy()
        end
        repeat
            task.wait()
            LocalPlayer.Character.RightHand.CFrame = CFrame.new(WristPosTarget.Character.HumanoidRootPart.Position) * CFrame.new(0,bugged.Aiming.WristPos.Distance,0)
            LocalPlayer.Character.RightHand.Transparency = 1
        until WristPosBind == false or bugged.Aiming.WristPos.Enabled == false
        if WristPosBind == false or bugged.Aiming.WristPos.Enabled == false then
...
mt.__namecall = newcclosure(function(...)
    local args = {...}
    if bugged.Aiming.Target.Enabled and TargetBindEnabled and getnamecallmethod() == "FireServer" and args[2] == "UpdateMousePos" then
        args[3] = TargetTarget.Character[bugged.Aiming.Target.Hitbox].Position+Vector3.new(0,bugged.Aiming.Target.Offset.Y,0)+(TargetTarget.Character[bugged.Aiming.Target.Hitbox].Velocity*bugged.Aiming.Target.Prediction)
...
RunService.RenderStepped:Connect(function ()
    if bugged.Aiming.Aimbot.VelocityResolver then
        pcall(function ()
    ...
    if bugged.Aiming.Aimbot.Enabled and AimbotBindEnabled and bugged.Aiming.Aimbot.Smoothing.Enabled == false then
        if bugged.Aiming.Aimbot.ReverseResolver == true then
            local zxc = CFrame.new(workspace.CurrentCamera.CFrame.p, AimbotTarget.Character[bugged.Aiming.Aimbot.Hitbox].Position - AimbotTarget.Character[bugged.Aiming.Aimbot.Hitbox].Velocity/bugged.Aiming.Aimbot.Prediction)
        ...
        if bugged.Aiming.Aimbot.ReverseResolver == false then
            local zxc = CFrame.new(workspace.CurrentCamera.CFrame.p, AimbotTarget.Character[bugged.Aiming.Aimbot.Hitbox].Position + AimbotTarget.Character[bugged.Aiming.Aimbot.Hitbox].Velocity/bugged.Aiming.Aimbot.Prediction)
            workspace.CurrentCamera.CFrame = workspace.CurrentCamera.CFrame:Lerp(zxc, 1, Enum.EasingStyle.Elastic, Enum.EasingDirection.InOut)
        end
    elseif bugged.Aiming.Aimbot.Enabled and AimbotBindEnabled and bugged.Aiming.Aimbot.Smoothing.Enabled == true then
        if bugged.Aiming.Aimbot.ReverseResolver == true then
            local zx = CFrame.new(workspace.CurrentCamera.CFrame.p, AimbotTarget.Character[bugged.Aiming.Aimbot.Hitbox].Position - AimbotTarget.Character[bugged.Aiming.Aimbot.Hitbox].Velocity/bugged.Aiming.Aimbot.Prediction)
            workspace.CurrentCamera.CFrame = workspace.CurrentCamera.CFrame:Lerp(zx, bugged.Aiming.Aimbot.Smoothing.Value, Enum.EasingStyle.Elastic, Enum.EasingDirection.InOut)
        end
        if bugged.Aiming.Aimbot.ReverseResolver == false then
            local zx = CFrame.new(workspace.CurrentCamera.CFrame.p, AimbotTarget.Character[bugged.Aiming.Aimbot.Hitbox].Position + AimbotTarget.Character[bugged.Aiming.Aimbot.Hitbox].Velocity/bugged.Aiming.Aimbot.Prediction)
            workspace.CurrentCamera.CFrame = workspace.CurrentCamera.CFrame:Lerp(zx, bugged.Aiming.Aimbot.Smoothing.Value, Enum.EasingStyle.Elastic, Enum.EasingDirection.InOut)
    ...
    if TargetBindEnabled and bugged.Aiming.Target.Enabled and TargetTarget.Character and TargetTarget.Character:FindFirstChild("HumanoidRootPart") then
        if bugged.Aiming.Target.LookAt then
            ...
            spawn(function ()
                if bugged.Aiming.Target.LookAt == false then
...
RunService.Stepped:Connect(function (param,FPS)
    if bugged.Aiming.TargetStrafe.Enabled then
        if bugged.Aiming.Target.Enabled and TargetBindEnabled then
            ...
            angle_Y = angle_Y + FPS / bugged.Aiming.TargetStrafe.Speed % 1
            LocalPlayer.Character.HumanoidRootPart.CFrame = CFrame.new(TargetTarget.Character.HumanoidRootPart.Position) * CFrame.Angles(0, 2 * math.pi * angle_Y, 0) * CFrame.new(0, bugged.Aiming.TargetStrafe.Height, bugged.Aiming.TargetStrafe.Distance)
        end
    end
end)
...
RunService.Heartbeat:Connect(function ()
    ...
    if bugged.Blatant.CFrame.Enabled then
        if LocalPlayer.Character.Humanoid.MoveDirection.Magnitude > 0 then
            for i = 1, bugged.Blatant.CFrame.Value do
                LocalPlayer.Character:TranslateBy(LocalPlayer.Character.Humanoid.MoveDirection)
            end
        end
    end
    if bugged.Blatant.Exploits.JumpCooldown then
        LocalPlayer.Character.Humanoid.UseJumpPower = false
    else 
        LocalPlayer.Character.Humanoid.UseJumpPower = true
    end
    if bugged.Blatant.Exploits.NoSlow then
        wait()
    ...
    if bugged.Blatant.Exploits.AutoStomp then
        game.ReplicatedStorage.MainEvent:FireServer("Stomp")
    end
    if bugged.Blatant.Exploits.AntiBag then
        if LocalPlayer.Character["Christmas_Sock"] then
            LocalPlayer.Character["Christmas_Sock"]:Destroy()
        end
    end
    if bugged.Blatant.AntiStomp.Enabled then
        if LocalPlayer.Character.Humanoid.Health <= 1 then
            if bugged.Blatant.AntiStomp.Type == "Remove Character" then
                for i, v in pairs(LocalPlayer.Character:GetDescendants()) do
                    if v:IsA("BasePart") then
                        v:Destroy()
                    end
                end
            elseif bugged.Blatant.AntiStomp.Type == "Remove Humanoid" then
                LocalPlayer.Character.Humanoid:Destroy()
            end
        end
    end
    if bugged.Blatant.FakeLag.Enabled then
        repeat
            wait()
            Toggles.AntiFlingEnabled:SetValue(true)
            wait(bugged.Blatant.FakeLag.Duration/4)
            Toggles.AntiFlingEnabled:SetValue(false)
        until bugged.Blatant.FakeLag.Enabled == false
    end
    if bugged.Blatant.AntiAim.VelocityUnderGround then
        VelocityUnderGroundAA()
    else
        LocalPlayer.Character.Humanoid.HipHeight = 2
    end
    if bugged.Blatant.AntiAim.RotVelocity.Enabled then
        RotVelocityAA()
    end
    if bugged.Blatant.GunMod.AutoReload then
...
Toggles.RainbowModeXD:OnChanged(function()
    bugged.UISettings.Rainbow = Toggles.RainbowModeXD.Value
end)
...
function VelocityUnderGroundAA()
    ...
    LocalPlayer.Character.HumanoidRootPart.Velocity = Vector3.new(oldVelocity.X, -bugged.Blatant.AntiAim.VelocityUnderGroundAmount, oldVelocity.Z)
...
function RotVelocityAA()
    LocalPlayer.Character.HumanoidRootPart.RotVelocity = Vector3.new(0, bugged.Blatant.AntiAim.RotVelocity.Value, 0)
end

task.spawn(function()
    while game:GetService("RunService").RenderStepped:Wait() do
        if bugged.UISettings.Rainbow then
